...
async function uploadToPublicHost(localFilePath) {
  ...
  const parentDir = path.basename(path.dirname(localFilePath)); // e.g. reel_1782357584420
  
  // Return the raw GitHub content URL directly
  // This serves the media directly from the GitHub CDN (100% uptime, zero Cloudflare/bot blocks)
  const githubUrl = `https://raw.githubusercontent.com/kireetiziggler/Insta_18plus_agent/main/data/posts/${parentDir}/${filename}`;
  console.log(`[PUBLISHER] Using GitHub raw CDN link: ${githubUrl}`);
  return githubUrl;
}
